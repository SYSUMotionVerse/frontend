...
describe('UniTrainingPageShell no-dock layout', () => {
  ...
  it('uses the same ambient treatment as growth only on ordinary docked pages', () => {
    ...
    expect(source).toContain('padding: 16rpx 32rpx 0;')
    expect(source).toContain('class="training-shell__dock-clearance"')
    expect(source).toContain('flex: 0 0 var(--floating-dock-content-clearance, 100px);')
