...
describe('ui review fixes', () => {
  ...
  it('keeps stable metadata on registration inputs and uses picker selectors for miniapp fields', () => {
    ...
    expect(file).not.toContain("handleNumericFieldInput('restingHeartRate'")
    expect(file).toContain(':range="collegeOptions"')
    expect(file).toContain('@change="handleCollegeChange"')
    expect(file).toContain(':range="educationOptions"')
  ...
  it('keeps the short post-training questionnaire as one calm, continuous check-in surface', () => {
    ...
    expect(shortQuestionnaireForm).toContain('short-questionnaire-form__scale-group')
    expect(shortQuestionnaireForm).toContain('short-questionnaire-form__status')
    expect(shortQuestionnaireForm).not.toContain('short-questionnaire-form__actions')
    expect(shortQuestionnaireForm).not.toContain('completedCount')
    expect(shortQuestionnaireForm).not.toContain('short-questionnaire-form__question-score')
    expect(shortQuestionnaireForm).toContain('提交并查看反馈')
    expect(shortQuestionnaireForm).toContain('background: #ff7777;')
    expect(shortQuestionnaireForm).toMatch(/\.short-questionnaire-form\s*\{[\s\S]*gap:\s*0;/)
    expect(shortQuestionnaireForm).toMatch(/\.short-questionnaire-form__intro\s*\{[\s\S]*margin-bottom:\s*32rpx;/)
    expect(shortQuestionnaireForm).toMatch(/\.short-questionnaire-form__question\s*\{[\s\S]*margin-bottom:\s*32rpx;/)
    expect(shortQuestionnaireForm).toMatch(/\.short-questionnaire-form__question-copy\s*\{[\s\S]*align-items:\s*center;/)
    expect(shortQuestionnaireForm).toMatch(/\.short-questionnaire-form__question-index\s*\{[\s\S]*font-size:\s*34rpx;/)
    expect(shortQuestionnaireForm).toMatch(/\.short-questionnaire-form__scale-group\s*\{[\s\S]*gap:\s*4rpx;/)
    expect(shortQuestionnaireForm).toMatch(/\.short-questionnaire-form__ticks\s*\{[\s\S]*padding:\s*0 18rpx;/)
    expect(shortQuestionnaireForm).toMatch(/\.short-questionnaire-form__slider\s*\{[\s\S]*width:\s*calc\(100% - 56rpx\);[\s\S]*margin:\s*-6rpx 28rpx 0;/)
    expect(shortQuestionnaireForm).toMatch(/\.short-questionnaire-form__scale-labels\s*\{[\s\S]*font-size:\s*25rpx;/)
